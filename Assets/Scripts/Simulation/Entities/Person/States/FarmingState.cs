...
public class FarmingState : PersonState
{
    private Point WorkBase;
    private Point WorkSpot;
    private FarmingShack Work;

    private WorkState State;
    private float HarvestRate = 1f;
    private float NextHarvestTick = 0f;

    public FarmingState(Person person) : base(person.gameObject)
    {
        _person = person;
    }
    ...
    public override void OnStateEnter()
    {
        ...
        Work = (FarmingShack)SimulationCore.Instance.AllWorkplaces[_person.Work];
        WorkSpot = Work.Farmland.RandomItem();
    ...
    private void MoveToResource()
    {
        ...
        if (gameObject.transform.position.IsSamePositionAs(GeneralUtility.GetLocalCenterOfCell(WorkSpot)))
        {
            ...
            State = WorkState.TendingToCrops;
